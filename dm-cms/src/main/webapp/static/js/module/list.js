    ...
    var list = {
        ...
        bindDisableAction: function() {
            $(".btn-disable").on("click", function() {
                ...
                if (ids.length <= 0) {
                    Dialog.warning("请先选择您要禁用的按钮");
                    return;
                }

                Dialog.confirm("禁用按钮", "确认要禁用这些按钮？",  function(result) {
                    if (result == true) {
                        $.ajax({
                            url: ctx + "/module/disable",
                            type: "POST",
                            data: {
                                id: ids
                            },
        ...
        bindEnableAction: function() {
            $(".btn-enable").on("click", function() {
                ...
                if (ids.length <= 0) {
                    Dialog.warning("请先选择您要启用的按钮");
                    return;
                }

                Dialog.confirm("启用按钮", "确认要启用这些按钮？",  function(result) {
                    if (result == true) {
                        $.ajax({
                            url: ctx + "/module/enable",
                            type: "POST",
                            data: {
                                id: ids
                            },
