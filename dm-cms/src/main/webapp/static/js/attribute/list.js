    ...
    var list = {
        ...
        initPagination: function() {
            Pagination.init({
                ...
                editBtn: {
                    url: ctx + "/attribute/edit/{id}"
                },
                deleteBtn: {
                    url: ctx + "/attribute/delete",
                    method: "POST",
        ...
        bindDisableAction: function() {
            $(".btn-disable").on("click", function() {
                ...
                if (ids.length <= 0) {
                    Dialog.warning("请先选择您要禁用的属性");
                    return;
                }

                Dialog.confirm("禁用属性", "确认要禁用这些属性？",  function(result) {
                    if (result == true) {
                        $.ajax({
                            url: ctx + "/attribute/disable",
                            type: "POST",
                            data: {
                                id: ids
                            },
        ...
        bindEnableAction: function() {
            $(".btn-enable").on("click", function() {
                ...
                if (ids.length <= 0) {
                    Dialog.warning("请先选择您要启用的属性");
                    return;
                }

                Dialog.confirm("启用属性", "确认要启用这些属性？",  function(result) {
                    if (result == true) {
                        $.ajax({
                            url: ctx + "/attribute/enable",
                            type: "POST",
                            data: {
                                id: ids
                            },
        ...
        redirectAddPage: function() {
            $(".btn-add").on("click", function() {
                location.href = ctx + "/attribute/edit";
            });
        }
    };
