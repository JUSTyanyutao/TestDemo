    ...
    var list = {
            ...
            $(".solveBtn").on("click",function(){
                $form.validate({
                    ...
                    submitHandler: function() {
                        ...
                        $.ajax({
                            url: window.ctx+"/goods/comment/approve?status="+status,
                            type: "POST",
                            data: $form.serialize(),
                            dataType: "JSON",
        ...
        recommendAction: function() {
            $(".btn-recommend-action").on("click",function(){
                ...
                if (ids.length <= 0) {
                    Dialog.warning("请先选择您要置顶的评价");
                    return;
                }
                Dialog.confirm("置顶评价", "确认要置顶这些评价？",  function(result) {
                    if (result == true) {
                        $.ajax({
                            url: window.ctx+"/goods/comment/recommend",
                            type: "POST",
                            data: {
                                id: ids
                            },
        ...
        disRecommendAction: function() {
            $(".btn-disRecommend-action").on("click",function(){
                ...
                if (ids.length <= 0) {
                    Dialog.warning("请先选择您要取消置顶的评价");
                    return;
                }
                Dialog.confirm("取消置顶评价", "确认要取消置顶这些评价？",  function(result) {
                    if (result == true) {
                        $.ajax({
                            url: window.ctx+"/goods/comment/disRecommend",
                            type: "POST",
                            data: {
                                id: ids
                            },
