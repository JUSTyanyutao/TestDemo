    ...
    <table class="table table-striped table-bordered table-hover">
        ...
        <c:forEach items="${page.content}" var="member" varStatus="status">
            <tr>
                ...
                <td>
                    <c:choose>
                        <c:when test="${member.profile.platform  eq 1}">微信</c:when>
                        <c:when test="${member.profile.platform  eq 2}">安卓</c:when>
                        <c:when test="${member.profile.platform  eq 3}">苹果</c:when>
                        <c:when test="${member.profile.platform  eq 4}">网站</c:when>
                        <c:when test="${member.profile.platform  eq 6}">手工添加</c:when>
                        <c:when test="${member.profile.platform  eq 7}">为知</c:when>
                    </c:choose>
                </td>
                <td><fmt:formatDate value="${member.registerTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                <td><fmt:formatDate value="${member.updateTime}" pattern="yyyy-MM-dd HH:mm"/></td>
