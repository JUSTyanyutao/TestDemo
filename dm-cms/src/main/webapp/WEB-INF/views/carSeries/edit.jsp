...
<c:set var="mainTitle" value="车 系列 管理" />
<c:set var="subTitle" value="${empty carSeries ? '添加系列' : '保存系列'}" />
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                           <form id="form-edit" role="form" method="POST"  enctype="multipart/form-data" class="form-horizontal" action="${ctx}/carSeries/save">
	                            <div class="row">
	                                <div class="col-md-12">
										<c:if test="${empty carSeries}">
											<input type="hidden" id="brandId" name="brandId" value="${id}">
										</c:if>
										<c:if test="${!empty carSeries}">
											<input type="hidden" id="brandId" name="brandId" value="${carSeries.brandId}">
										</c:if>
										<input type="hidden"  name="id" value="${carSeries.id}">

										<div class="form-group">
											<label class="col-md-2 control-label">系列名称:</label>
											<div class="col-md-8">
												<input type="text" name="name" id="name" placeholder="请输入系列名称"
	                                                class="form-control" value="${carSeries.name}">
	                                         </div>
										</div>
										...
										<c:if test="${!empty carSeries}">
											<div class="form-group" >
												...
												<div class="col-md-7">
													<input type="radio"
													<c:if test="${carSeries.status eq 1}">
														   checked = "true"
													</c:if>
														   value="1"  name="status">&nbsp;启用&nbsp;&nbsp;&nbsp;
													<input type="radio"
															<c:if test="${carSeries.status eq 0}">
																checked = "true"
															</c:if>
	                             ...
	                             <button type="submit" class="btn btn-submit btn-sm btn-primary"
                                                data-loading-text="${empty carSeries ? "添加" : "保存"}中...">
                                            ${empty carSeries ? "添 加" : "保存"}
                                 </button>
...
<script src="${ctx}/static/js/carSeries/edit.js"></script>
</body>
</html>